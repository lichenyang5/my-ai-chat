...
process 是 Node.js 的全局对象，代表当前运行的 Node.js 进程，包含了进程的各种信息和方法。

process.cwd() 才是返回当前工作目录的方法。

简单理解：

process = 整个进程对象（包含环境变量、命令行参数、内存使用等信息）
...
Node.js 环境下使用 `process.stdout.write()` 实现打字机效果，关键是**不换行**。
...
浏览器没有 `process` 对象，需要通过**字符串拼接 + DOM 更新**实现打字机效果。
